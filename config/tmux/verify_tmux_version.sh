...
verify_tmux_version() {
    ...
    if [[ $tmux_version -eq "3.1c"]] ; then
        source-file $tmux_home/tmux_3.1c.conf
        exit
    if [[ $(echo "$tmux_version >= 2.5" | bc) -eq 1 ]] ; then
        source-file $tmux_home/tmux_3.1c.conf
        exit
    elif [[ $(echo "$tmux_version >= 2.1" | bc) -eq 1 ]] ; then
        source-file $tmux_home/tmux_2.1_to_2.5.conf
        exit
    elif [[ $(echo "$tmux_version >= 1.9" | bc) -eq 1 ]] ; then
        source-file $tmux_home/tmux_1.9_to_2.1.conf
        exit
    else
        source-file $tmux_home/tmux_1.9_down.conf
        exit
    fi
}
